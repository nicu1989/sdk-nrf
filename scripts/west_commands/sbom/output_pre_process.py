...
def assign_application_dependencies(data: Data, files_by_package: dict):
    '''Populate package dependencies for detected application roots.'''
    ...
    if not application_packages:
        return
    module_packages = packages_for_roots(data.module_roots, files_by_package)
    if not module_packages:
        module_packages = {pkg_id for pkg_id in files_by_package if pkg_id in data.packages}
    for package_id in application_packages:
        if package_id not in data.packages:
            continue
        deps = sorted(pkg for pkg in module_packages if pkg not in ('', package_id))
